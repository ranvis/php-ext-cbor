...
h\'2222222222222222 33333333\'', cdec($data, CBOR_EDN, ['byte_space' => 8, 'byte_wrap' => 16, 'indent' => 2]));
    ...
    eq(' /ERROR:' . CBOR_ERROR_SYNTAX . ' (syntax)/', cdec('5f60ff', CBOR_EDN));
    // break in definite length
    eq('[ /ERROR:' . CBOR_ERROR_SYNTAX . ' (syntax)/', cdec('81ff', CBOR_EDN));

    // simple values
    eq('simple(0)', cdec('e0', CBOR_EDN));
    eq('simple(19)', cdec('f3', CBOR_EDN));
    eq(' /ERROR:' . CBOR_ERROR_MALFORMED_DATA . ' (malformed data)/', cdec('f817', CBOR_EDN)); // not well-formed
    eq(' /ERROR:' . CBOR_ERROR_MALFORMED_DATA . ' (malformed data)/', cdec('f818', CBOR_EDN)); // reserved
    eq(' /ERROR:' . CBOR_ERROR_MALFORMED_DATA . ' (malformed data)/', cdec('f81f', CBOR_EDN)); // reserved
